...
const PublishCourse = () => {
  ...
  const handelPublish = async () => {
    if (
      ...
      (course?.status === COURSE_STATUS.DRAFT && getValues("public") === false)    //no updatiion no need for api call
    ) {
      goToMyCourses();
      setLoading(false);
      dispatch(setStep(1));
      ...
      <div className="rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="ml-auto flex max-w-max items-center gap-x-4">
            <button
              disabled={loading}   //iska mtlb jb ye button clicked ho tb koi action na le paye
              onClick={goBack}
              type="button"
